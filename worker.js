...
self.onmessage = function (e) {
  ...
  const processAllStats = (allMatches, playersSet, isFullHistory = true) => {
    ...
    allMatches.forEach(({ g, i: originalIndex }) => {
      ...
      const avg1 =
        p1A.length > 0
          ? p1A.reduce((s, p) => s + getElo(p), 0) / p1A.length
          : 1000; // Robust gegen leere Teams
      const avg2 =
        p2A.length > 0
          ? p2A.reduce((s, p) => s + getElo(p), 0) / p2A.length
          : 1000; // Robust gegen leere Teams
      ...
      const avgMatchEloDelta = Math.round(
        totalEloTransferredForMatch / (winners.length || 1),
      );
      ...
      if (winnerKey && loserKey && avgMatchEloDelta > 0) {
        ...
        aggregates.eloTransfers[transferKey] =
          (aggregates.eloTransfers[transferKey] || 0) + avgMatchEloDelta;
      }
  ...
  try {
    ...
    self.postMessage({
      ...
      careerStatsBeforeToday: careerStatsBeforeToday,
    });
  } catch (err) {
